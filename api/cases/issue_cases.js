    ...
    try {
      ...
      if (organization_id) {
        ...
        cases = await sql`
          SELECT ic.*
          FROM issue_cases ic
          JOIN case_organization co ON ic.issue_cases_id = co.issue_cases_id
      ...
      const merged = cases.map((c) => {
        ...
        const co = caseOrgs.find((co) => co.issue_cases_id === c.issue_cases_id);
    ...
    try {
      ...
      if (organization_ids && organization_ids.length > 0) {
        for (const orgId of organization_ids) {
          await sql`
            INSERT INTO case_organization (issue_cases_id, organization_id, is_viewed)
